...
void My_MpiScatter(	void * sbuf, int scount, MPI_Datatype stype, 
...
int main(int argc, char ** argv)
{
    ...
    int RecvBuf;
    int *SendBuf = (int *)malloc(N_of_processes * sizeof(int));
	...
	{
	    My_MpiScatter(	SendBuf, 1, MPI_INT, 
					...
					void * rbuf, int rcount, MPI_Datatype rtype, int root, MPI_Comm comm)
{
	...
	if(ProcRank == root)
	{
		...
		{
			if(i != root)
				MPI_Send(	(void *)&(((int *)sbuf)[i]), scount, stype, 
							i, 0, comm);
			else
				*(int *)rbuf = ((int *)sbuf)[i];
		}
	...
	{
		MPI_Recv(	rbuf, rcount, rtype, root,
					0, comm, &Status);
